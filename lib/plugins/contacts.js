  ...
  return {
    ...
    isBlocking: function ({ source, dest }, cb) {
      if (values && values[source]) {
        ...
        view.get((err, graph) => {
          if (err) return cb(err)
          const blocking = graph && graph[source] && graph[source][dest] === false
          cb(null, blocking)
        })
      }
    },
    ...
    ignoreStream: function ({ live }) {
      const stream = PullPushAbort()
      ...
      pull(
        ...
        pull.drain((msg) => {
          ...
          const isPrivate = msg.value && msg.value.meta && msg.value.meta.private
          ...
          const value = isPrivate ? !!content.blocking : null
    ...
    stateStream: function ({ feedId, live = false, reverse = false }) {
      const stream = PullPushAbort()
      ...
      const queryStream = reverse ? ssb.backlinks.read({
        query: [{ $filter: {
          dest: feedId,
          value: {
            content: {
      ...
      pull(
        ...
        pull.drain((msg) => {
          ...
          const isPrivate = msg.value && msg.value.meta && msg.value.meta.private && msg.value.author === ssb.id

          if (!isPrivate) {
            const content = msg.value.content
            const contact = reverse ? msg.value.author : content.contact
            const value = getContactState(msg.value.content)
    ...
    replicateStream: function ({ throttle = 5000, live }) {
      ...
      const update = () => {
        ...
        const resolvedValues = resolveValues(values, ssb.id)
