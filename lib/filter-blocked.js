...
module.exports = function (blockSources, { isBlocking, checkRoot = false, useRootAuthorBlocks = false }) {
  ...
  return pull(
    paramap((msg, cb) => {
      ...
      async.map(checks, isBlocking, (err, res) => {
        ...
        for(let i = 0; i < checks.length; i++) {
          if (res[i]) {
            // I shouldn't be modifying the original msg in a (para)map I guess
            // but :shrug: (? or will I break stuff)
            msg.isBlocked = true
            msg.blockedBy = {
              id: checks[i].source,
    ...
    pullFilterAsync((msg, cb) => {
      ...
      cb(null, !(msg.isBlocked && msg.blockedBy.role === 'me'))
    }, 10)
  )
}
