...
exports.create = function (api) {
  ...
  return nest('profile.sheet.edit', function ({ usePreview = true } = {}) {
    ...
    api.sheet.display(close => {
      ...
      const currentPublicWebHosting = api.about.obs.valueFrom(id, 'publicWebHosting', id)
      ...
      const chosenPublicWebHosting = Value(currentPublicWebHosting())
            ...
            h('div.main', [
              ...
              h('label', [
                h('input', {
                  disabled: publishing,
                  type: 'checkbox',
                  hooks: [CheckBoxHook(chosenPublicWebHosting)]
                }), ' ', i18n('Public Web Hosting - Allows your messages to be viewed in Public Web Hosting providers.')
              ])
            ])
          ])
        ]),
      ...
      function save () {
        ...
        if (chosenPublicWebHosting() !== currentPublicWebHosting()) update.publicWebHosting = chosenPublicWebHosting()
...
function CheckBoxHook (obs) {
  return function (element) {
    element.value = obs() === true // false values can be false, 'false', null, and ostensibly undefined
    element.onchange = (ev) => {
      obs.set(ev.target.checked)
    }
  }
}
