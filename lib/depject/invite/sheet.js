...
exports.create = function (api) {
  ...
  return nest('invite.sheet', function () {
    api.sheet.display(close => {
      ...
      const showInviteInfo = Value(false)
      ...
      return {
        content: h('div', {
          ...
          }, [when(showInviteInfo, inviteInfo, i18n('By default, Patchwork will only see other users that are on the same local area network as you.'))]),
          h('div', [
              ...
              publishStatus.set(api.invite.async.accept(input.value.trim(), (err, info) => {
                if (err) {
                  ...
                  if (info) {
                    ...
                    inviteInfo.set(JSON.stringify(info, null, 2))
                    showInviteInfo.set(true)
                  } else {
                    close()
                  }
                }
