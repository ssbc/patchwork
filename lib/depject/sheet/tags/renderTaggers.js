...
exports.create = function (api) {
  ...
  return nest('sheet.tags.renderTaggers', function (msgId, tagId) {
    ...
    const taggerIds = ScuttleTag.obs.messageTaggers(msgId, tagId)
    const currentFilter = Value()
    const taggerLookup = mutantLookup(taggerIds, (id) => {
      return [id, api.about.obs.name(id)]
    })
    const filteredIds = computed([taggerIds, taggerLookup, currentFilter], (ids, lookup, filter) => {
